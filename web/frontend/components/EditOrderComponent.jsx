import {Spinner,TextField,  Toast,Frame,  Card, PageActions, Layout, Button, Modal, TextContainer,  IndexTable, useIndexResourceState, SkeletonThumbnail} from '@shopify/polaris';
...
const addProductVariant = async() =>{
   ...
   let result = productId.substring(29,);
    ...
    const requestOptions = {
      ...
      const response = await fetch("/api/addProduct/" + orderId + "/" + result , requestOptions);
      if (response.ok) {
...
const [showProducts, setShowProducts] = useState(false)
  const changeAmount = async() =>{
   ...
   if(quantity === originalQuantity)
   {
        ...
        setToastProps({
            content: "Cannot change the quantity of a product that was originally zero",  
            error: true,
          });
   }
   else{
    ...
    const requestOptions = {
      ...
      const response = await fetch("/api/changeAmount/" + orderId + "/" + lineItemId + "/" + quantity, requestOptions);
      if (response.ok) {
  ...
  const handleChangeQuantity = useCallback(() => setActiveQuantity(!activeQuantity), [activeQuantity]);
  ...
  return (
    <Frame>
      ...
      <Layout.Section >
      <Card title={orderName ? `Order Details for Order ${orderName}` : 'Pick an Order' }primaryFooterAction={{
          content: "Save",
                ...
                renderItem={(item) => {
                  ...
                  return (
                    <ResourceList.Item
                      id={id}
                      url={url}
                      media={hasPicture ? <Thumbnail source={image}
                      alt="picture of product"/> : <SkeletonThumbnail />}
