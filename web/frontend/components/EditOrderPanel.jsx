...
export function EditOrderPanel (){
  ...
  useEffect(() => {
    ...
    const [taxesChanged, setTaxesChanged] = useState(false);
  ...
  const getOrderSubtotal = async () => {
    fetch("/api/tax/subtotal/" + orderId)
    .then((response) => response.json())
    .then((json) => {
  ...
  <Frame>
    ...
    >
      ...
      {productId && (
        ...
        handleTaxesChanged={handleTaxesChanged}
        taxPrice={taxPrice}
        />
        )  
}
